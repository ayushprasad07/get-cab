...
export default function DriverLoginPage() {
  ...
  const { loginDriver, error, clearError } = useAuth();
  ...
  return (
    <>
      <style>{`
        ...
        .card-animate  { animation: fadeUp 0.7s cubic-bezier(.16,1,.3,1) 0.1s both; }
        ...
        <div className="relative w-full max-w-md">
          ...
          <Card className="card-animate bg-zinc-950 pt-8 border border-white/10 rounded-2xl overflow-hidden">
            <CardContent className="p-7">
              ...
              {/* Error */}
              {error && (
                <div className="error-shake mb-5 flex items-start gap-3 bg-red-500/8 border border-red-500/20 rounded-xl px-4 py-3">
              ...
              <form onSubmit={handleSubmit} className="space-y-5 form-animate">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="btn-primary w-full bg-white !text-black hover:bg-white font-semibold h-11 text-sm tracking-wide mt-2"
                  style={{ color: "black" }}
                >
                  {loading ? (
              ...
              <div className="space-y-3 text-center">
                <p className="text-neutral-600 text-sm">
                  No account yet?{" "}
                  <Link href="/auth/driver-register" className="link-hover font-medium">
                    Create one
                  </Link>
                </p>
                <p className="text-neutral-600 text-sm">
                  Looking for a ride?{" "}
                  <Link href="/auth/user-login" className="link-hover font-medium">
                    User login
                  </Link>
                </p>
              </div>
